...
To get started on `riptide_turtle` go into `src\riptide_turtle` and read the README for your desired language (Python or C++). The Python files are already in `src\riptide_turtle\src\py` and the C++ files are in `src\riptide_turtle\src\cpp`.
