...
using namespace std::chrono_literals;  // Use C++ timing literals to define intervals like 1s for 1
                                       // second

class TurtleNode : public rclcpp::Node { // Define a new class 'TurtleNode' that inherits from the
                                         // ROS 2 'Node' class
public:
    ...
    TurtleNode() : Node("turtle_node") {
        ...
        RCLCPP_INFO(
            get_logger(), "Turtle Node started!"); // Log a message to confirm the node has started
    }
    ...
    void publishTwist() {
        ...
        RCLCPP_INFO(
            get_logger(), "Published twist message!"); // Log a message after publishing the twist
    }

    rclcpp::Publisher<geometry_msgs::msg::Twist>::SharedPtr
        publisher; // Define the publisher as a shared pointer and provide its message type
...
int main(int argc, char * argv[]) {
    ...
    rclcpp::spin(std::make_shared<TurtleNode>()); // Create an instance of the TurtleNode and keep
                                                  // it running until manually interrupted
